            ...
            <div class="col-lg-12">
                <table class="table mt-4 text-center text-capitalize">
                    <thead class="thead-light">
                        <tr>
                            <th>
                                <h1> Data- {{ missionskill.skill_id }} </h1>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>
                                <div class="d-flex">
                                    <h5>Title:</h5>
                                    <h5 class="card-title ms-2  text-info">{{ missionskill.skill_name }}</h5>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td class="m-3">
                                <div class="d-flex">
                                    <h5>Status:</h5>
                                    <h5 class="card-title ms-2" :class="getStatusLabel(missionskill.status)">{{ getStatusLabel(missionskill.status)
                                    }}</h5>
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td>
                                <div class="card-body btns text-center ">
                                    <router-link to="/missionskill" class="nav-link text-white"
...
export default {
    ...
    created() {
        ...
        console.log('skill_id:', this.$route.params.skill_id);
        axios.get(`/api/missionskill/${this.$route.params.skill_id}`)
            .then(response => {
